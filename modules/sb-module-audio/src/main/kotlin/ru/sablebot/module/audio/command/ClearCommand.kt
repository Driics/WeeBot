...
class ClearCommand(
    ...
    override fun command() = slashCommand(
        ...
        CommandCategory.MUSIC, UUID.fromString("a1b2c3d4-e5f6-7890-abcd-ef1234567813")
    ) {
        executor = ClearExecutor()
    }

    inner class ClearExecutor : SlashCommandExecutor() {
        override suspend fun execute(context: ApplicationCommandContext, args: SlashCommandArguments) {
            try {
                ...
                val removed = playerService.clearQueue(guild)
                if (removed > 0) {
                    context.reply(ephemeral = false, "Cleared **$removed** track(s) from the queue.")
                } else {
                    context.reply(ephemeral = true, "The queue is already empty.")
                }
